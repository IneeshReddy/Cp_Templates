...
struct SegTree{
    ...
    void upd(int lx, int rx, int i, int l, int r, int val){
        push(i, l, r);
        ...
        if(l >= lx && r <= rx){
             tree[i] = mul(tree[i], val);

             if(l != r){
                 lazy[2 * i] = mul(lazy[2 * i], val);
                 lazy[2 * i + 1] = mul(lazy[2 * i + 1], val);
             }
        ...
        upd(lx, rx, 2 * i, l, m, val);
        upd(lx, rx, 2 * i + 1, m + 1, r, val);
    ...
    void upd(int l, int r, int val){ upd(l, r, 1, 0, size, val); };
};
